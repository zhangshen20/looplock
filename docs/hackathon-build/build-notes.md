...
## 27 August 2026 — Item 10 producer dry run and narration timing

- The separate demo-producer task completed one event-locked technical dry run through the real reference path. BetBurst and LuckyMirror blocked locally; LuckyMirror was quarantined before upload; the same opaque package-event ID reached terminal Android and Firestore state; `AGENT_TIGHTENED` was accepted only after local validation; offline blocking persisted; and the visible `UNLOCK` fixture returned `ACTION_NOT_ALLOWED` without changing the commitment end.
- The rehearsal exposed and resolved two capture-order hazards: disconnecting only the proxy can trigger WorkManager retry backoff until the five-minute commitment expires, and a prior block overlay remains visible until **Return to LoopLock** is tapped. The final run sheet now disables emulator networking before install, enables it before the first worker attempt, and explicitly clears each overlay between fixture launches.
